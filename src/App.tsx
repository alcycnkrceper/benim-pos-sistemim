...
export default function App() {
  ...
  const handleTahsilat = async (customer) => {
    ...
    if (tutar && !isNaN(tutar)) {
      await updateDoc(doc(db, 'customers', customer.id), { balance: (customer.balance || 0) - parseFloat(tutar) });
  ...
  const rawTotal = cart.reduce((t, i) => t + ((i.grossPrice || 0) * i.qty), 0);
  const totalCost = cart.reduce((t, i) => t + ((i.costPrice || 0) * i.qty), 0);
  ...
  const finishSale = async (method) => {
    ...
    const saleData = { 
      ...
      totalCost: totalCost, total: finalTotal, method, 
    ...
    if (method === 'Veresiye' && activeCustomer) await updateDoc(doc(db, 'customers', activeCustomer.id), { balance: (activeCustomer.balance || 0) + finalTotal });
    
    setLastSale({ id: ref.id, ...saleData });
    setCart([]); setCartCustomer(''); setDiscountPct(''); setIsVeresiyeModalOpen(false);
  };
  ...
  // --- EXCEL (.XLSX) PARAŞÜT SATIŞ FATURASI AKTARIMI (TEMİZ V5 SÜRÜM) ---
  const exportParasutInvoices = () => {
    
    // GEREKSİZ UYARI METİNLERİ VE BOŞ SATIRLAR SİLİNDİ. SADECE TEMİZ BAŞLIKLAR:
    const headers = [
      "Müşteri Ünvanı", "Fatura İsmi", "Fatura Tarihi", "Döviz Cinsi", "Döviz Kuru", 
      "Vade Tarihi", "Tahsilat TL Karşılığı", "Fatura Türü", "Fatura Seri", "Fatura Sıra No", 
      "Kategori", "Hizmet/Ürün", "Hizmet/Ürün Açıklaması", "Çıkış Deposu", "Miktar", 
      "Birim Fiyatı", "İndirim Tutarı", "KDV Oranı", "ÖİV Oranı", "Konaklama Vergisi Oranı"
    ];

    const data = [headers]; // Excel'in 1. Satırı direkt başlıklar oldu.

    let invoiceCounter = 1;

    sales.forEach(sale => {
      if (sale.method === 'Tahsilat') return; // Tahsilatlar faturaya girmez.
  ...
  const exportProducts = () => {
    const data = products.map(p => [(p.name || '').replace(/,/g, ''), p.barcode || '', p.unit || '', p.costPrice || 0, p.grossPrice || 0]);
  ...
  const netCommercialProfit = totalIncome - totalCogs - totalExpense;
              ...
              <div className="flex-1 overflow-y-auto grid grid-cols-2 lg:grid-cols-4 gap-4 content-start">
                {products.filter(p => (p.name || '').toLowerCase().includes(searchQuery.toLowerCase()) || (p.barcode || '').includes(searchQuery)).map(p => (
                  <button key={p.id} onClick={() => addToCart(p)} className="bg-zinc-900 border border-zinc-800 p-5 rounded-2xl text-left hover:border-emerald-500 hover:bg-zinc-800/50 transition-all flex flex-col justify-between h-36 group">
                    <span className="font-bold text-zinc-200 group-hover:text-emerald-400 line-clamp-2">{p.name || 'İsimsiz Ürün'}</span>
                    <div className="flex justify-between items-end"><span className="text-[10px] bg-zinc-800 px-2 py-1 rounded text-zinc-500">{p.unit || 'Adet'}</span><span className="text-2xl font-black text-white">₺{p.grossPrice || 0}</span></div>
                  </button>
                ))}
              </div>
            </div>
            ...
            <div className="w-[420px] bg-zinc-900 border-l border-zinc-800 flex flex-col shadow-2xl">
              <div className="p-5 border-b border-zinc-800 bg-zinc-950/20">
                ...
                <select value={cartCustomer} onChange={e => setCartCustomer(e.target.value)} className="w-full bg-zinc-950 border border-zinc-700 p-3 rounded-xl text-white outline-none focus:border-emerald-500 text-sm font-bold">
                  ...
                  {customers.map(c => <option key={c.id} value={c.id}>{c.name} (Bakiye: ₺{(c.balance || 0).toFixed(2)})</option>)}
                </select>
              </div>
              ...
              <div className="flex-1 overflow-y-auto p-4 space-y-3">
                {cart.map(item => (
                  <div key={item.id} className="bg-zinc-950 p-4 rounded-2xl border border-zinc-800 flex justify-between items-center group animate-in fade-in zoom-in duration-200">
                    <div className="flex-1"><div className="text-sm font-bold text-zinc-300 line-clamp-1">{item.name}</div><div className="text-emerald-500 font-black">₺{((item.grossPrice || 0) * item.qty).toFixed(2)}</div></div>
              ...
              <div className="p-6 bg-zinc-950 border-t border-zinc-800">
                <div className="flex items-center justify-between mb-4 bg-zinc-900 p-3 rounded-xl border border-zinc-800">
                  ...
                  <div className="flex items-center gap-1"><span className="text-zinc-500">%</span><input type="number" min="0" max="100" value={discountPct} onChange={e => setDiscountPct(e.target.value)} placeholder="0" className="w-16 bg-zinc-950 border border-zinc-700 rounded-lg p-1.5 text-center text-white outline-none focus:border-emerald-500 font-bold"/></div>
                </div>
                ...
                <button onClick={() => setIsVeresiyeModalOpen(true)} className="w-full bg-emerald-500 py-5 rounded-2xl font-black text-zinc-950 hover:bg-emerald-400 active:scale-95 transition-all shadow-lg shadow-emerald-500/20">VERESİYE YAZ</button>
              </div>
            </div>
          </div>
        )}
                ...
                <tbody className="divide-y divide-zinc-800/50">
                  {products.map(p => (
                    <tr key={p.id} className="hover:bg-zinc-800/30 transition-colors">
                      ...
                      <td className="p-5 text-right font-medium text-blue-400 text-sm">₺{(p.costPrice || 0).toFixed(2)}</td>
                      <td className="p-5 text-right font-black text-white text-lg font-mono">₺{(p.grossPrice || 0).toFixed(2)}</td>
              ...
              {customers.map(c => (
                <div key={c.id} className="bg-zinc-900 border border-zinc-800 p-6 rounded-3xl flex justify-between items-center hover:border-emerald-500/30 transition-all">
                  ...
                  <div className="text-right space-y-3">
                    <div className={`text-2xl font-black font-mono ${(c.balance||0) > 0 ? 'text-red-500' : (c.balance||0) < 0 ? 'text-emerald-500' : 'text-zinc-600'}`}>
                      {(c.balance||0) > 0 ? `+ ₺${(c.balance||0).toFixed(2)}` : (c.balance||0) < 0 ? `- ₺${Math.abs(c.balance||0).toFixed(2)}` : 'BORCU YOK'}
                    </div>
                ...
                <div className="flex-1 overflow-y-auto space-y-4 pr-2">
                  {sales.slice().reverse().map((s, idx) => (
                    <div key={idx} className="bg-zinc-950 p-5 rounded-2xl border border-zinc-800 flex justify-between items-center group">
                      <div><div className="text-xl font-black text-emerald-400">₺{(s.total || 0).toFixed(2)}</div><div className="text-[10px] text-zinc-600 font-mono mt-1 uppercase">{s.date}</div></div>
                      <div className="text-right"><div className="font-bold text-zinc-300">{s.customerName}</div><div className="text-[10px] bg-zinc-900 px-2 py-0.5 rounded border border-zinc-800 inline-block mt-1 text-zinc-500">{s.method} {(s.discountPct || 0) > 0 ? `(%${s.discountPct} İndirim)` : ''}</div></div>
                    </div>
                  ))}
                </div>
              </div>
          ...
          <table className="w-full text-left mb-10">
            ...
            <tbody className="divide-y-2 divide-gray-300">
              {lastSale.items.map((i,idx) => (
                <tr key={idx}><td className="py-4 font-bold">{i.name}</td><td className="py-4 text-center font-black">{i.qty}</td><td className="py-4 text-right font-medium">₺{(i.grossPrice||0).toFixed(2)}</td><td className="py-4 text-right font-black">₺{((i.grossPrice||0) * i.qty).toFixed(2)}</td></tr>
              ))}
            </tbody>
          </table>
          <div className="flex justify-end">
            <div className="w-80 border-t-4 border-black pt-4">
              <div className="flex justify-between text-xl font-bold text-gray-600 mb-2"><span>Ara Toplam:</span><span>₺{(lastSale.subTotal||0).toFixed(2)}</span></div>
              {(lastSale.discountAmount||0) > 0 && <div className="flex justify-between text-xl font-bold text-gray-600 mb-4 border-b-2 border-gray-300 pb-2"><span>İskonto (%{lastSale.discountPct}):</span><span>- ₺{(lastSale.discountAmount||0).toFixed(2)}</span></div>}
              <div className="flex justify-between text-4xl font-black mt-2"><span>TOPLAM:</span><span>₺{(lastSale.total||0).toFixed(2)}</span></div>
            </div>
          </div>
